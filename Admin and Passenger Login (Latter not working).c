...
flight_node* addToEmpty(char* flight_number, char* airline_service, char* flight_destination, char* flight_departure, char* departure_date, char* departure_time, int flight_duration, int seat_available);
flight_node* addAtEnd(flight_node* tail, char* flight_number, char* airline_service, char* flight_destination, char* flight_departure, char* departure_date, char* departure_time, int flight_duration, int seat_available);
...
void calculateArrivalTime(flight_node* flight) {
    ...
    strptime(flight->departure_time, "%H:%M", &tm_departure);
    time_departure = mktime(&tm_departure);

    time_t time_arrival = time_departure + (flight->flight_duration * 60);

    struct tm* tm_arrival = localtime(&time_arrival);
    strftime(flight->arrival_time, sizeof(flight->arrival_time), "%H:%M", tm_arrival);
}
...
flight_node* addAtEnd(flight_node* tail, char* flight_number, char* airline_service, char* flight_destination, char* flight_departure, char* departure_date, char* departure_time, int flight_duration, int seat_available) {
    if (tail == NULL) {
        return addToEmpty(flight_number, airline_service, flight_destination, flight_departure, departure_date, departure_time, flight_duration, seat_available);
    }
...
void viewByDestination(flight_node* tail, char* destination) {
    ...
    int found = 0;
    while (temp != NULL) {
        if (strcmp(temp->flight_destination, destination) == 0) {
            printf("Flight Number: %s\n", temp->flight_number);
            printf("Airline Service: %s\n", temp->airline_service);
            printf("Destination: %s\n", temp->flight_destination);
            printf("Departure: %s\n", temp->flight_departure);
...
void createNewFlightInfo(flight_node** tail) {
    ...
    *tail = addAtEnd(*tail, flight_number, airline_service, flight_destination, flight_departure, departure_date, departure_time, flight_duration, seat_available);
